...
export async function handler(event, context) {
  ...
  try {
    ...
    const sireCompatibility = await fetch(`${process.env.URL}/.netlify/functions/breeding-program-compatibility`, {
      method: 'POST',
    ...
    const damCompatibility = await fetch(`${process.env.URL}/.netlify/functions/breeding-program-compatibility`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // Call COI calculator
    const coiResult = await fetch(`${process.env.URL}/.netlify/functions/coi-calculator`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const colorPrediction = await fetch(`${process.env.URL}/.netlify/functions/color-prediction`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sireId, damId })
    }).then(res => res.json());
