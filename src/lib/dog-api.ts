...
// Interface for Dog API response
interface DogImage {
  id: string;
  url: string;
  width: number;
 ...
 */
export async function fetchRandomDogImages(limit = 1): Promise<DogImage[]> {
  try {
 ...
 */
export async function downloadDogImage(imageUrl: string): Promise<string> {
  try {
 ...
 */
export async function downloadRandomDogImages(count = 5): Promise<string[]> {
  try {
 ...
 */
export async function getSavedDogImages(): Promise<string[]> {
  try {
    // Ensure the directory exists
    await ensureDogImagesDir();
