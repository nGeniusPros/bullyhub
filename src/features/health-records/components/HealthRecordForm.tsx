...
import { HealthRecord, RECORD_TYPE_OPTIONS } from "../types";
...
export function HealthRecordForm({ dogId, onSuccess, existingRecord }: HealthRecordFormProps) {
  ...
  // Get the queries
  const healthRecordQueries = useHealthRecordQueries();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingRecord) {
        ...
        await healthRecordQueries.updateHealthRecord(
          existingRecord.id,
          recordData,
          files || undefined
        );
        ...
        // Create new record
        await healthRecordQueries.createHealthRecord(
          recordData as any,
          files || undefined
        );
      }
