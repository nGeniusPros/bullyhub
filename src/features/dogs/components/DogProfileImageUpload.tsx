...
export function DogProfileImageUpload({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Use the dog queries to upload the image
      const dogQueries = useDogQueries();
      const imageUrl = await dogQueries.uploadDogImage(dogId, file);
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      // Use the dog queries to remove the image
      const dogQueries = useDogQueries();
      await dogQueries.removeDogImage(dogId);
